...
const canSubmit = computed(() => {
  return isValidAmount.value && !isSubmitting.value
})
...
async function handleSubmit() {
  ...
  const result = await transactionStore.deposit(accountId.value, {
  ...
  if (result.success) {
    successMessage.value = `Deposit of ${formatCurrency(amount.value)} completed successfully!`
    amount.value = ''
    description.value = ''
    await accountStore.fetchAccounts()
  } else {
    errorMessage.value = result.error_message || 'Deposit failed'
  }
